...
def prepare():
    ...
    profile = plistlib.loads(subprocess.check_output([
        'security', 'cms', '-D', '-i', str(temporary / 'syscredi.mobileprovision')
    ]))
    entitlements = profile['Entitlements']
    bundle_id = 'mz.ladans.syscredi'
    ...
    if entitlements['application-identifier'].split('.', 1)[1] != bundle_id:
        raise ValueError('The provisioning profile must match mz.ladans.syscredi.')
    destination = Path.home() / 'Library/Developer/Xcode/UserData/Provisioning Profiles'
    destination.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(temporary / 'syscredi.mobileprovision', destination / f"{profile['UUID']}.mobileprovision")
    ...
    options = {'method': 'app-store-connect', 'destination': 'export',
               ...
               'signingCertificate': 'Apple Distribution', 'teamID': team,
    ...
    Path('defines.json').write_text(json.dumps({'GOOGLE_IOS_CLIENT_ID': client_id, 'GOOGLE_SERVER_CLIENT_ID': os.environ.get('GOOGLE_SERVER_CLIENT_ID', '')}))
    ...
    for entry in info.get('CFBundleURLTypes', []):
        entry['CFBundleURLSchemes'] = [scheme for scheme in entry.get('CFBundleURLSchemes', []) if scheme != previous_scheme]
    info.setdefault('CFBundleURLTypes', []).append({'CFBundleTypeRole': 'Editor', 'CFBundleURLSchemes': ['.'.join(reversed(client_id.split('.')))]})
